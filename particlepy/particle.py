...
class BaseParticle:
    """
    This is the basic particle class. It has no size and is only being used to create subclasses.
    ...
    :ivar twisted_progress: A variable from 1 to 0 to measure how much the particle has progressed to its disappearing, inversion of progress
    ...
    def __init__(self, position, velocity, color, alpha=255, lighting_color=None):
        ...
        self.progress = 0
        # TODO: Change twisted_progress to inverted_progress
    ...
    def update(self, start_size, size, delta_time=1, gravity=0):
        """
        Updates position, velocity, progress and twisted_progress according to delta_time and gravity:
...
class Circle(BaseParticle):
    ...
    def update(self, start_size, size, delta_time=1, gravity=0):
        """
        Updates position, velocity, size, progress and twisted_progress according to delta_time and gravity:
        gravity lets particles fall down and delta_time smooths out the movement
...
class Rect(BaseParticle):
    ...
    def draw(self, surface):
        ...
        """
        if self.alive:
            gfxdraw.box(surface, (int(self.position[0]) - int(self.size / 2), int(self.position[1]) - int(self.size / 2),
